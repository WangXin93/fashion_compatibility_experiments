...
def train():
    for epoch in range(1, epochs + 1):
        ...
        for batch_num, input_data in enumerate(train_loader, 1):
            ...
            all_loss = f_loss + b_loss + 0 * emb_loss
        ...
        torch.save(f_rnn.state_dict(), os.path.join("f_rnn{}.pth".format(comment)))
        torch.save(b_rnn.state_dict(), os.path.join("b_rnn{}.pth".format(comment)))
        torch.save(
            embedding.state_dict(), os.path.join("embedding{}.pth".format(comment))
        )
        torch.save(
            image_embedding.state_dict(), os.path.join("image_embedding{}.pth".format(comment))
        )
        torch.save(
            encoder_cnn.state_dict(), os.path.join("encoder_cnn{}.pth".format(comment))
        )
